...
async function runGeneticAlgorithm(startGeneration = 0, initialPopulation = null, initialBest = null) {
	if (!maze) {
	...
	const totalGens = startGeneration + numGenerations;
	const meanFitness = stats.fitnessSum / (populationSize * totalGens);
	const variance = (stats.fitnessSquaredSum / (populationSize * totalGens)) - (meanFitness * meanFitness);
	...
	lastRunState = {
		population: population,
		bestIndividual: bestIndividualOfAllTime,
		generation: startGeneration + numGenerations,
		maze: JSON.stringify(maze)
	};
...
document.addEventListener('DOMContentLoaded', () => {
	const algorithms = ['dfs', 'prim', 'kruskal', 'recursiveBacktracker', 'sidewinder', 'eller', 'wilson', 'growingTree', 'huntAndKill'];
